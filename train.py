    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch % 1 == 0:
            ...
            if mDice > best_dice:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(epoch) + '_{:.4f}'.format(best_dice) + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(epoch) + '_{:.4f}'.format(best_dice) + '.pth'
                if main_process():
