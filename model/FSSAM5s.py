...
class OneModel(nn.Module):
    ...
    def generate_prior_proto(self, query_feat_high, final_supp_list, mask_list, fts_size, normalize=False):
        ...
        for i, tmp_supp_feat in enumerate(final_supp_list):
            ...
            fg_sim_max = fg_sim.max(1)[0]  # b
            fg_sim_maxs.append(fg_sim_max.unsqueeze(-1))  # b, 1
            ...
            fg_sim = fg_sim.view(bsize, 1, sp_sz, sp_sz)  # b, 1, h, w
            bg_sim = bg_sim.view(bsize, 1, sp_sz, sp_sz)  # b, 1, h, w

            fg_sim = F.interpolate(fg_sim, size=fts_size, mode='bilinear', align_corners=True)  # b, 1, H, W
            bg_sim = F.interpolate(bg_sim, size=fts_size, mode='bilinear', align_corners=True)  # b, 1, H, W
            fg_list.append(fg_sim)
            bg_list.append(bg_sim)
        fg_corr = torch.cat(fg_list, 1)  # b, s, H, W
        bg_corr = torch.cat(bg_list, 1)  # b, s, H, W
        corr = (fg_corr - bg_corr)       # b, s, H, W
        corr[corr < 0] = 0
        ...
        if normalize:
            ...
            corr = corr.view(bsize, len(final_supp_list), *fts_size)  # b, s, H, W
    ...
    def generate_prior_mem(self, qry_feat, supp_fg, supp_mask, fts_size):
        ...
        if shot == 1:  # 1 shot
            ...
            supp_fg = supp_fg.view(bsize, shot, ch_sz, sp_sz, sp_sz)  # b, s, c, h, w
            supp_mask = supp_mask.view(bsize, shot, 1, sp_sz, sp_sz)  # b, s, 1, h, w
            fg_sims = []
            for i in range(shot):
                fg_supp_feat = Weighted_GAP(supp_fg[:, i, ...], supp_mask[:, i, ...])  # b, c, 1, 1
                fg_sim = self.cos_sim(qry_feat, fg_supp_feat, cosine_eps)
                fg_sim = fg_sim.max(1)[0].view(bsize, sp_sz * sp_sz)
                fg_sim = (fg_sim - fg_sim.min(1)[0].unsqueeze(1)) / (
                            fg_sim.max(1)[0].unsqueeze(1) - fg_sim.min(1)[0].unsqueeze(1) + cosine_eps)
            ...
            fg_sim = torch.cat(fg_sims, dim=1)
            fg_sim = fg_sim.mean(1, True)  # b, 1, h, w
        return fg_sim
    ...
    def forward(self, x, s_x, s_y, y_m, cat_idx=None, priors=None):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            s_mask[s_mask == 255] = 0  # b, shot, H, W
            ...
            if priors is None:
                with torch.no_grad():
                    ...
                    qry_feat = qry_feats[-1]  # b, c, 40, 40
                    sup_feat = sup_feats[-1]  # b*s, c, 40, 40
                    
                    size = qry_feat.size()[-2:]  # 40, 40

                    if shot > 1:
                        sup_feat = sup_feat.view(b, shot, -1, *size)  # b, s, c, h, w
                        sup_mask = s_mask.view(b, shot, 1, h, w)  # b, s, 1, H, W
                        sup_feat_list, sup_mask_list = [], []
                        for i in range(shot):
                            sup_feat_list.append(sup_feat[:, i, ...])  # [b, c, h, w]
                            sup_mask_list.append(F.interpolate(sup_mask[:, i, ...], size=size, mode='bilinear', align_corners=True))  # [b, 1, h, w]
                        
                        # generate dinov2 prior masks
                        size = (h, w)  # H, W
                        normalize = True
                        prior_fg, _, prior_ae, _, _ = self.generate_prior_proto(qry_feat, sup_feat_list, sup_mask_list, size, normalize=normalize)
                        priors = torch.cat([prior_fg.unsqueeze(2), prior_ae.unsqueeze(2)], dim=2)  # b, s, 2, H, W - for saving
                    else:
                        # interpolate support mask
                        sup_mask = F.interpolate(s_mask.float(), size=size, mode='bilinear', align_corners=True)  # b, 1, h, w
                        
                        # generate dinov2 prior masks
                        size = (h, w)  # 512, 512
                        normalize = True
                        prior_fg, _, prior_ae, _, _ = self.generate_prior_proto(qry_feat, [sup_feat], [sup_mask], size, normalize=normalize)
                        prior_fg = F.interpolate(prior_fg, size=size, mode='bilinear', align_corners=True)  # b, 1, H, W
                        prior_ae = F.interpolate(prior_ae, size=size, mode='bilinear', align_corners=True)  # b, 1, H, W
                        priors = torch.cat([prior_fg, prior_ae], dim=1)  # b, 2, H, W - for saving
            else:
                priors = priors  # b, 2, H, W / b, s, 2, H, W
                if shot > 1:
                    prior_fg = priors[:, :, 0, ...]  # b, s, H, W
                    prior_ae = priors[:, :, 1, ...]  # b, s, H, W
                else:
                    prior_fg = priors[:, 0, ...].unsqueeze(1)  # b, 1, H, W
                    prior_ae = priors[:, 1, ...].unsqueeze(1)  # b, 1, H, W
            prior_fg = prior_fg.mean(1, True)
            prior_ae = prior_ae.mean(1, True)

            # ========================================
            # SAM2 - video mode
            # ========================================
            x = load_video_frames_from_data(x, offload_video_to_cpu=False)      # b, 3, H, W
            s_x = load_video_frames_from_data(s_x, offload_video_to_cpu=False)  # b*s, 3, H, W
            with torch.no_grad():
                # obtain query and support features
                _, _, qry_feats, qry_poss, qry_sizes = self.sam2.get_image_feature_batch(x)    # b, c, h, w
                _, _, sup_feats, sup_poss, sup_sizes = self.sam2.get_image_feature_batch(s_x)  # b*s, c, h, w

            # add support prompt - gt mask
            sup_fg = s_mask.view(-1, 1, h, w)  # b, 1, H, W / b*s, 1, H, W
            (sup_fg_preds, sup_fg_obj_ptrs, sup_fg_mem_feats, sup_fg_mem_poss) = self.sam2.add_new_mask_batch(sup_feats, sup_sizes, sup_fg)    # sup fg mem, b, c, h, w / b*s, c, h, w

            # add query prompt - prior mask
            (qry_fg_preds, qry_fg_obj_ptrs, qry_fg_mem_feats, qry_fg_mem_poss) = self.sam2.add_new_mask_batch(qry_feats, qry_sizes, prior_fg)  # qry fg mem, b, c, h, w
            (qry_ae_preds, qry_ae_obj_ptrs, qry_ae_mem_feats, qry_ae_mem_poss) = self.sam2.add_new_mask_batch(qry_feats, qry_sizes, prior_ae)  # qry ae mem, b, c, h, w

            # ========================================
            # iterative memory refinement
            # ========================================
            sup_fg = F.interpolate(sup_fg, size=qry_sizes[-1], mode='bilinear', align_corners=True)      # b, 1, h, w / b*s, 1, h, w
            prior_ae = F.interpolate(prior_ae, size=qry_sizes[-1], mode='bilinear', align_corners=True)  # b, 1, h, w
            prior_fg = F.interpolate(prior_fg, size=qry_sizes[-1], mode='bilinear', align_corners=True)  # b, 1, h, w
